...
Last activity: 2026-05-15 - Completed quick task 260515-uid: Go live with real-customer SMS — three test-mode gates removed, Pi restarted, manual-send-only

Stopped at: SMS test-mode gates fully removed. Customer phones from the queue now reach Twilio. SCHEDULER_ENABLED stays off — operator must click Send-now / Resend per row in admin UI. 6 customers (jobs 143, 147, 152, 162, 171, 312) had their messages redirected to Jas+Marco between 2026-05-13 and 2026-05-15 and never received them; recovery via /queue/resend awaits operator decision. Twilio prod migration (260424-oyk WhatsApp→SMS branch) was unrelated path — current deploy already uses TWILIO_FROM=+19256033934 long code direct.
Resume file: None
