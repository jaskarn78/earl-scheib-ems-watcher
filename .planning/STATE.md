...
stopped_at: Completed quick task 260505-q2t — closed-RO detection shipped end-to-end. Watcher now overrides DocumentStatus to "C" when AD2.RO_CMPDATE/DATE_OUT is set AND TTL.G_TTL_AMT > 0, routing closed work orders to the server's review-job branch instead of the estimate 24h+72h branch. Source aca49db, installer release 3012fd7 (SHA256[:16] da3029f88f659ea2, was 621e7c7cb08354e5).
...
Last session: 2026-05-05T19:10:00Z
Stopped at: Quick task 260505-q2t complete end-to-end. Closed-RO detection live in installer (release commit 3012fd7, SHA256[:16] da3029f88f659ea2). When CCC ONE writes a bundle whose .ad2 has RO_CMPDATE/DATE_OUT populated and .ttl G_TTL_AMT > 0, the watcher now POSTs DocumentStatus="C" → server schedules a review-request SMS instead of estimate follow-ups. Pending operator action: remove `update_paused` sentinel on Marco's machine (WIN-8I9KME32KLC) so his self-update loop pulls the new installer within ~7min.
Resume file: None
