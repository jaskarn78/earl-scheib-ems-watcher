...
stopped_at: 260526-wnc shipped + deployed live — DEV-MODE SCHEDULER_ENABLED env gate replaced by a persistent, Marco-controllable auto-send toggle (app_settings DB table, default OFF seeded from env on first boot, DB is sole source of truth thereafter). New CF-Access-gated POST /earlscheibconcord/auto-send; admin UI toggle replaces the red DEV banner. Pi deployed: jobs.db seeded auto_send_enabled='0', /diagnostic scheduler_enabled=False — auto-send OFF, no behavior change until Marco flips it. Also fixed root-owned .git/working-tree drift on the Pi (chowned /opt/esw/app to esw) so future esw deploys fast-forward cleanly.
last_updated: "2026-05-26T23:55:00Z"
progress:
  total_phases: 0
  completed_phases: 0
  total_plans: 0
...
| 260515-uid | Go live with real-customer SMS — all three test-mode gates removed. (1) `app.py:98` `SMS_ALLOWLIST = {"+15308450190","+19254215772"}` → `set()` (commit 22c809c, pushed). (2) Pi `/opt/esw/app/.env` `TEST_PHONE_OVERRIDE` + `TEST_PHONE_RECIPIENTS` commented out (kept for rollback; backup `.env.bak-pre-golive-1778882351`). Pi git-pulled + earlscheib.service restarted (PID 39520, listening 0.0.0.0:8200). Running-process env confirmed via `/proc/<pid>/environ`: TEST_PHONE_* absent, SCHEDULER_ENABLED absent. Preconditions: A2P 10DLC APPROVED for +19256033934, Marco signed off. **Scheduler stays off per user — manual /queue/send-now only.** Historical impact: 6 customer messages between 2026-05-13 and 2026-05-15 (jobs 143, 147, 152, 162, 171, 312 — Smalls/Mazaheri/Deavers/Nelson/Harrison/Knight) were marked sent in queue but actual Twilio delivery was fanned out to Jas + Marco. To recover, /queue/resend on each row (operator decision). | 2026-05-15 | 22c809c | [260515-uid-go-live-with-real-customer-sms-remove-te](./quick/260515-uid-go-live-with-real-customer-sms-remove-te/) |
| 260526-wnc | Persistent auto-send toggle replaces the DEV-MODE `SCHEDULER_ENABLED` env gate. New generic `app_settings` key-value DB table + `get_auto_send_enabled()`/`set_auto_send_enabled()` (mirror UKK's defensive `get_schedule_enabled` shape, but default-OFF — conservative for a live SMS server). First-boot seed: `init_db` runs `INSERT OR IGNORE` from the env var (`SCHEDULER_ENABLED`, absent on Pi → `'0'`/OFF); env is seed-only, DB is sole source of truth thereafter (proven by `test_env_ignored_after_seed`). All 4 read sites swapped to fresh DB reads (scheduler_loop gate, startup log, gated-off log text, `/diagnostic` — JSON key `scheduler_enabled` kept for UI back-compat); `/queue/send-now` stays ungated (SPN-03 contract preserved). New CF-Access-gated `POST /earlscheibconcord/auto-send` with strict bool validation (rejects ints/strings/missing/bad-JSON → 400). UI: red DEV banner replaced by an oxblood-on-cream `#auto-send-toggle` wired to the `/diagnostic` poll + POST-on-user-change (hidden in local Go admin where `scheduler_enabled` isn't boolean — public-UI-only, no Go files touched); `make sync-ui` → byte-identical `ui_public/`. 17 gate tests green; live endpoint smoke-tested locally (seed OFF, persist ON/OFF, 400s). **Deployed live to earlscheib-pi**: chowned `/opt/esw/app` esw (cleared root-owned `.git`/worktree drift from prior `root@` deploys), `git reset --hard e4fb196`, `systemctl restart earlscheib.service` (PID 131817 active); `jobs.db` seeded `auto_send_enabled='0'`, `/diagnostic` → `scheduler_enabled=False`. **Auto-send OFF — Marco flips it from the UI; no SMS until he does.** 3 commits: e2a44ff tests, 5792126 server, b50d5d2 UI. Server-side only — no installer rebuild. | 2026-05-26 | 5792126 / b50d5d2 | [260526-wnc-persistent-auto-send-toggle](./quick/260526-wnc-persistent-auto-send-toggle/) |
| 260515-ush | Replace admin Logs tab with Twilio-backed Messages view — new `GET /earlscheibconcord/twilio-messages` reads Twilio Messages API server-side (60s cache); UI rewrite shows `When·↑↓·Customer·Phone·Type·Status·Body·SID` with chip filters (status/direction/range) + freshness indicator. Customer enrichment: body-match against `sms_log` (authoritative for `send_sms` path including fan-out residue) → forward-prefix parse `From +NNN:` for messages to Marco's number → unambiguous-phone fallback. Folded in LAE-regression hot-fix: 8 browser endpoints (`/templates`, `/schedules`, `/sms-log`, `/queue/resend`, `/queue/uncancel`, `/reset-test-jobs`, PUT `/templates/{}`, PUT `/schedules/{}`) were 401ing because LAE only updated `/queue` — `_validate_auth` removed from all 8, function deleted (no callers). 6 auth tests rewritten to match no-origin-auth contract. Round-trip verified live (Jas reply → TwiML Bin → Marco, ~0s latency). 2 catch-up forwards sent to Marco: Jim Nelson 5/15 10:31 AM (`+19255884764`) and Anthony Smalls 5/13 4:13 PM (`+19254359099`) — the inbound replies that previously hit Twilio's dev auto-reply before TwiML Bin was wired. | 2026-05-15 | 734b2da | [260515-ush-replace-logs-tab-with-twilio-backed-view](./quick/260515-ush-replace-logs-tab-with-twilio-backed-view/) |

## Session Continuity

Last session: 2026-05-26
Last activity: 2026-05-26 - Completed quick task 260526-wnc: persistent auto-send toggle replaces DEV-MODE env gate; deployed live to earlscheib-pi with auto-send seeded OFF

Stopped at: 260526-wnc deployed and verified live. The auto-send scheduler is now controlled by a persistent DB toggle (`app_settings.auto_send_enabled`) instead of the `SCHEDULER_ENABLED` env var. On the Pi the row seeded to `'0'` (OFF) and `/diagnostic` reports `scheduler_enabled=False` — Marco turns auto-send ON from the admin UI toggle at `/earlscheib`; until he does, only manual `/queue/send-now` fires SMS. The env var is now a first-boot seed only and is ignored once the row exists. Resolved a Pi-side ownership drift along the way: `/opt/esw/app/.git` + working tree had root-owned files from earlier `root@earlscheib-pi` deploys, blocking `esw` fast-forward pulls — `chown -R esw:esw /opt/esw/app` fixed it; future deploys as `esw` will `git pull --ff-only` cleanly. Two pre-existing test-fixture failures remain out of scope: `test_get_templates_placeholder_catalog` and `test_get_templates_sample_row_from_pending_job`.

Prior stop (260515-ush): USH-01 deployed and verified live. Messages tab on `/earlscheib#logs` reads from Twilio (not local sms_log) with body-match-first customer enrichment. TwiML Bin forwards inbound SMS to Marco (+19254215772) at `From +NNN: <body>`; round-trip latency ~0s. Tammie Knight (#312) resend delivered at 03:42 PM PT today; the other 5 fan-out era customers (jobs 143/147/152/162/171 — Smalls/Mazaheri/Deavers/Nelson/Harrison) ALREADY got their actual messages via the original sends (Twilio API confirmed delivered) — only Tammie needed the resend. Marco received 2 catch-up forwards for inbound replies that hit Twilio's dev auto-reply before TwiML Bin was wired. SCHEDULER_ENABLED still off — manual /queue/send-now only. Two pre-existing test-fixture failures unrelated to this work: `test_get_templates_placeholder_catalog` (missing `short_model` in placeholder list) and `test_get_templates_sample_row_from_pending_job` (test expects old shop_name).
Resume file: None
